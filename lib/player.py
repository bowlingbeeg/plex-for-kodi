...
class ZidooPlayerHandler(BasePlayerHandler):
    ...
    def getVideoWatched(self, ref=None):
        return self.getVideoPlayedFac(ref=ref) >= self.playedThresholdPerc / 100.0
    ...
    def onPlayBackStopped(self):
        ...
        # show post play if possible, if an item has been watched (90% by Plex standards)
        util.DEBUG_LOG("ZidooHandler: played-threshold: {}%/{}%".format(int(self.videoPlayedFac * 100), int(self.playedThresholdPerc)))
...
class ZidooPlayer(xbmc.Player, signalsmixin.SignalsMixin):
    ...
    def play(self, *args, **kwargs):
        ...
        if self.handler and isinstance(self.handler, ZidooPlayerHandler):
            ...
            url = util.addURLParams(url, {
                'PlexToZidoo-ViewOffset': self.handler.seekOnStart if self.handler.seekOnStart is not None else 0,
    ...
    def playVideo(self, video, resume=False, force_update=False, session_id=None, handler=None):
        ...
        if handler and isinstance(handler, ZidooPlayerHandler):
            ...
            self.handler = ZidooPlayerHandler(self, session_id or self.sessionID)
    ...
    def _playVideo(self, offset=0, force_update=False, playerObject=None, session_id=None):
        self.sessionID = session_id or self.sessionID
        self.trigger('new.video', video=self.video)
        self.trigger(
            'change.background',
    ...
    def playVideoPlaylist(self, playlist, resume=False, handler=None, session_id=None):
        ...
        if handler and isinstance(handler, ZidooPlayerHandler):
            util.DEBUG_LOG("ZidooPlayVideoPlaylist: Reusing old handler: {}", handler)
            self.handler = handler
            self.handler.reused = True
            #self.handler.queuingNext = True
            #self.handler.seekOnStart = 0
            ...
            while not util.MONITOR.abortRequested() and not self._closed:
                ...
                if zidooStatusFull is not None:
                    ...
                    while self.started and not util.MONITOR.abortRequested() and not self._closed:
                        ...
                        if timeJump:
                            self.handler.updateNowPlaying(state=self.STATE_PAUSED) # The PAUSED state should actually force an update
                        else:
                            self.handler.updateNowPlaying()
